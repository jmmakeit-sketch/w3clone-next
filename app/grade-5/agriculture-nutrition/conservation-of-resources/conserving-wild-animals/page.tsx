...
export default function ConservingWildAnimalsDynamic() {
  const data = grade5AgriNutritionContent["conservation-of-resources"].subStrands["conserving-wild-animals"];

  return (
    <div className="p-6 max-w-4xl mx-auto space-y-6">
      <div className="border-b pb-4">
        <span className="text-xs font-bold text-amber-600 bg-amber-50 px-2.5 py-1 rounded-md tracking-wider">
          SUB-STRAND {data.id}
        </span>
          ...
          <ul className="space-y-1.5">
            {data.objectives.map((obj, i) => (
              <li key={i} className="text-sm text-slate-600 flex items-start">
                <span className="text-amber-500 mr-2">✓</span> {obj}
              </li>
            ))}
          </ul>
        </div>
