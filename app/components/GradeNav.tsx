...
export default function GradeNav() {
  ...
  // When route changes, update pinned to match active grade
  useEffect(() => {
    setPinnedIndex(activeGrade >= 0 ? activeGrade : null);
    setOpenIndex(activeGrade >= 0 ? activeGrade : null);
    window.dispatchEvent(new CustomEvent("gradenav:open", { detail: activeGrade >= 0 ? activeGrade : null }));
  }, [pathname]);
  ...
  function handleMouseLeave() {
    hoverTimeout.current = setTimeout(() => {
      // Return to pinned grade dropdown, or close if no pinned
      setOpenIndex(pinnedIndex);
      window.dispatchEvent(new CustomEvent("gradenav:open", { detail: pinnedIndex }));
    }, 200);
  }
  ...
  return (
    <>
            ...
            return (
              <button key={g.href}
                ...
                style={{
                  ...
                  color: "#fff",
                  background: isActive ? "#04AA6D" : "#000",
                  borderBottom: isActive ? "3px solid #038a58" : "3px solid transparent",
      ...
      {/* Click outside to close — only when no active grade */}
      {openGrade && !activeGrade && (
      ...
      {openGrade && (
        <div
          ...
          onMouseLeave={handleMouseLeave}
          style={{ position: "fixed", left: 0, top: "132px", width: "228px", background: "#f1f1f1", borderTop: `3px solid ${openGrade.color}`, borderRight: "1px solid #ddd", zIndex: 9998, height: "calc(100vh - 132px)", overflowY: "auto", scrollbarWidth: "none" }}>
